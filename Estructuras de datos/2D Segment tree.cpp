...
// MAXNODES = (2^k) - 1, tal que, 2^(k-1) >= MAXN.
const int MAXNODES = 1023; // k = 10, MAXN <= 1000.
...
struct st {
  ...
  st(st& st1, st& st2, int tam) : sz{tam} {
    t.resize(MAXNODES);
  ...
  st(const vector<int>& arr) : sz{(int) arr.size()} {
    t.resize(MAXNODES);
    build(0, sz - 1, 0, arr);
  }
  void build(int s, int e, int p, const vector<int>& arr) {
    if (s == e) {
      t[p] = {arr[s], arr[s]};
      return;
    }
  ...
  void set(int up, const ii& uv, int s, int e, int p) {
    if (up >= s && up <= e) {
      if (s == e) {
        t[p] = uv;
      } else {
...
struct st2d {
  int N,M;
  vector<st> t;
  st2d(int a, int b) : N{a}, M{b} { // matriz de axb con elementos neutros
    t.resize(MAXNODES);
    build(0, N - 1, 0);
  }
  st2d(const vector<vector<int>>& mat) : N{(int) mat.size()}, M{(int) mat[0].size()} {
    // construye a partir de mat
    t.resize(MAXNODES);
    build(0, N - 1, 0, mat);
  }
  void build(int s, int e, int p, const vector<vector<int>>& mat) {
    if (s == e) {
      t[p] = st(mat[s]);
      return;
    }
  ...
  void upd(int upx, int upy, const ii& uv, int s, int e, int p) {
    if(upx >= s && upx <= e) {
      if (s == e) {
        t[p].set(upy, uv);
      } else {
